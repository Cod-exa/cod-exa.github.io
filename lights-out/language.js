/**
 * Lights Out: Night Atlas — Multi-language Engine & i18n Integration
...
(() => {
  ...
  function applyLanguage(lang) {
    ...
    if (title) {
      document.title = `${title} — Lights Out: Night Atlas — CodExa`;
    }
  ...
  window.addEventListener('codexa:languagechange', event => {
    const lang = normalizeLang(event.detail && event.detail.lang);
